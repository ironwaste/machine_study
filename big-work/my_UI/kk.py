...
class T(sch.Ui_MainWindow,QtWidgets.QMainWindow) :
    ...
    def open_png(self) :
        ...
        # self.filename = filename # 成员变量 - 存储
    ...
    def get_testdata(self):
        ...
        for i in range(0, len(test_files)):
            ...
            test_set[i, :] = testarr
            # print(str(true_label) + "  :  " + test_files[i])
        # print(true_label_set)
        # print(true_label_set)
    ...
    def get_split_data(self):
        '''将后缀为05和15的文件找出来作为测试集,其他文件为训练集'''
        txt_list = listdir(self.txt_path)

        for txt in txt_list:
            try:
                # if (txt.split(".")[0].split("_")[1] == "05") or (txt.split(".")[0].split("_")[1] == "19"):
                shutil.move(self.txt_path + "/" + txt, self.test_path)
                # else:
                #     shutil.move(self.txt_path + "/" + txt, self.train_path)
            except:
                pass
